...
def fetchCoverurl(driver, waiting_increment = 0.1):
  waiting_sec = 0
  while(True):
    if (waiting_sec > GENERAL_WAITING_SEC):
      coverurl = None
      break
    try:
      coverurl = driver.find_element_by_xpath("//div[@id='main-pic']/img").get_attribute("src")
      break
    except selenium.common.exceptions.NoSuchElementException:
      waiting_sec = waiting_sec + waiting_increment
      time.sleep(waiting_increment)
...
#      book_name = fetchBookname(driver)
    ...
    image_url = fetchCoverurl(driver)
    ...
    bookinfo["sellers"][seller_name]["image_url"] = image_url
    driver.back()
    time.sleep(HARD_WAITING_SEC)
...
def writeBookinfoToFile(bookinfo, outputfile):
  ...
  for seller_name in sellers:
    ...
    output_string += '|'
    try:
      output_string += str(bookinfo["sellers"][seller_name]["image_url"])
    except KeyError:
      output_string += "None"
    output_string += ","
  output_string += '];\n'
...
if __name__=='__main__':
  ...
  booknames = fetcherToolbox.readFileLines(booknames_filename)
