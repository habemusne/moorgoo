  ...
  while (True):
    ...
    if (currentPageBooknames == None):
      ...
      for i in range(0, len(currentPageBooknames)):
        try:
          writeBooknameToFile(currentPageBooknames[i], outputfile)
          continue
        except Exception, ex:
          logging.exception("****Exception in method writeBooknameToFile()****")
...
def writeBooknameToFile(bookname, outputfile):
  outputfile.write(bookname.encode("utf-8") + "\n")
...
if __name__=='__main__':
  ...
  outputfilename = "BooknamesOnPage" + str(start_from_page) + "_" + str(end_at_page)
